...
@allure.step("Checks that the file is available at the specified URL.")
def assert_file_is_accessible(url: str):
    ...
    """
    logger.info("Checks that the file is available at the specified URL.")
...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
    Проверяет, что ответ на получение файла с некорректным значением file_id
...
def assert_file_not_found_response(actual: InternalErrorResponseSchema):
    """
    Функция для проверки ошибки, если не найден на сервере.
